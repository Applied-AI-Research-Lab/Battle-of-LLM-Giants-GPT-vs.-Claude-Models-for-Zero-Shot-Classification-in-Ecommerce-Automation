...
params = {
    ...
    'prediction_column': 'claude_3.5_haiku_prediction',  # Specifies the column where predictions will be stored.
    'pre_path': 'Datasets/',  # Specifies the base directory path where dataset files are located.
    'data_set': 'split_datasets/dataset_split_8.csv',  # Defines the path to the CSV dataset file.
    'prompt_array': {'json_file': 'split_datasets/categories_8.json'},  # Can be an empty array for simple projects.
    # Defines the system prompt that describes the task.
    'system': 'You are an AI assistant specializing in product classification for an ecommerce platform.',
    ...
    'prompt': 'Your task is to analyze a given product title and assign it to the most appropriate category from a predefined list. [json] Provide your final classification in the following JSON format without explanations: {"category": "chosen_category_name"}',
    'feature_col': 'title',  # Specifies the column in the dataset containing the text input/feature for predictions.
